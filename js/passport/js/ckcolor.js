(function() {
	...
	var colorCk = function(_option) {
		...
		this.each(function(i, m) {
			/*var opt = {
				"width": $(m).attr("data-width"),
				"height": $(m).attr("data-height"),
				"unckedUrl": $(m).attr("data-unckedUrl"),
				"ckedUrl": $(m).attr("data-ckedUrl"),
				"color": $(m).attr("data-color"),
				"background": $(m).attr("data-background"),
				"border": $(m).attr("data-border")
			}
			if ($(m).attr("data-width") || $(m).attr("data-height") || $(m).attr("data-unckedUrl") || $(m).attr("data-ckedUrl") || $(m).attr("data-color") || $(m).attr("data-background") || $(m).attr("data-border")) {
				colorCk.setings = $.extend(colorCk.seting, opt);
			} else {
				colorCk.setings = $.extend(opt, colorCk.seting);
			}*/
			//创建复选框
