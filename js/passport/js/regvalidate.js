...
(function($) {
	...
	validateObj.RegExs = {
		...
		vphone: /^1[34578]\d{9}$/,
		vemail: /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
		vname: /^(?![0-9]+$)[a-zA-Z0-9_]{3,16}$/, //(?![0-9]+$)/,//(?![0-9]+$)/,//^[^\s](?![0-9]+$)[a-zA-Z][a-zA-Z0-9_]{2,15}$/,//(?![0-9]+$)
	...
	validateObj.validateForm = function() {
			...
			if (inputVtype.length == 1) {
				if (inputVtype[0] == "vrepwd") {
					...
					if (!_this.is(":hidden")) {
						...
						//当不输入验证类型或者没有该验证类型就免去验证
						if (zzobj != undefined && zzobj != "") {
							if (!zzobj.test(_thisValue)) {
								...
								if (_this.attr("valid") != undefined && _this.attr("valid") != "") {
									if (!validateObj._ajax(_this)) {
										validateObj.unpassStyle(_this[0], _this.attr("v-ayncError") ? _this.attr("v-ayncError") : _this.attr("placeholder")+"已存在");
									}
								}
							}
						} else {
				...
				if (!_flag) {
					...
					//替换手机和邮箱的
					validateObj.showhidecaptcha(_this, (_o == "vphone") ? true : false);
					if (_this.attr("valid") != undefined && _this.attr("valid") != ""){
						if (!validateObj._ajax(_this)) {
							validateObj.unpassStyle(_this[0], _this.attr("v-ayncError") ? _this.attr("v-ayncError") : _this.attr("placeholder")+"已存在");
						}
	...
	validateObj._ajax = function(m) {
		var ayncbool = false;
		//判断是否包含路径如果没有或者为空就不做任何请求验证 
		$.ajax({
			type: "post",
			url: $(m).attr("valid"),
			data:{name:$(m).val()},
...
//			crossDomain:true,
	...
	validateObj.showhidecaptcha = function(_m, bool) {
		var next_obj=$(_m).parent().parent().next();
		var _next_obj =next_obj.next();
		if (_next_obj.find("input").size() > 1) {
			if (bool) {
				next_obj.show();
				next_obj.find(".leftblock").find("input").show();
				...
				next_obj.hide();
				next_obj.find(".leftblock").find("input").hide();
